    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.classifyId) {
            this.$http({
              url: this.$http.adornUrl(`/arct/classify/info/${this.dataForm.classifyId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.classify = data.classify.classify
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/arct/classify/${!this.dataForm.classifyId ? 'save' : 'update'}`),
              method: 'post',
